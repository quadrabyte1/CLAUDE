"""
render_v15.py  —  Gemma's matplotlib render of deck_plan v15  (Page 2: Decking Plan)
Produces deck_plan.png at >=2000 px wide, white background.

v15 = v14 + ONE change (drawio only):
  - Replaced ASCII <pre>-style stair elevation cells in the drawio with
    native drawio shapes (rectangles, lines, dimension arrows, text labels).
  - The matplotlib PNG render is identical to v14 except for the version badge.
  - No structural or dimensional changes from v14.

v14 = v13 + THREE changes (carried forward, see render_v14.py for detail):

Change 1 — Deck depth pulled in 3" (Option C, confirmed 2026-05-06):
  - Framing depth: 16'-3" → 16'-0"  (fits clean 16-ft 2x10 stock)
  - Overall finished surface depth: 16'-6" → 16'-3"  (PF outer edge)
  - Outer pier row: y=16'-3" → y=16'-0"  (now at FRAME_D)
  - Far PF outer edge: y=16'-3"  (= new OVERALL_D)
  - Fascia outer face: y=16'-0.67"  (same 0.67" fascia thickness)
  - 3" exposed patio strip shown at far edge (y=16'-3" to y=16'-6")
  - PF cantilever past rim still 3" exact
  - PF cantilever past fascia still 2.33" exact

Change 2 — Both stair elevation views confirmed present:
  - LEFT-EDGE STAIR side elevation (was orphaned reference in drawio in v13;
    elevation diagram was present in render but drawio had only a text stub —
    proper graphical elevation restored/confirmed in v14 render)
  - FAR-EDGE STEP side elevation (added in v13, carried forward unchanged)

Change 3 — Full v13→v14 version sweep:
  - Version badges, title block, all "v13" references → v14
...
FRAME_D    = 16.0           # v14: framing depth = 16'-0" (was 16.25 = 16'-3" in v13)
...
OVERALL_D  = FRAME_D + PF_OVR        # v14: 16.25 ft = 16'-3"

# Original patio depth (fixed — does not change)
PATIO_D    = 16.5           # 16'-6" — patio extends 3" past deck in v14
...
# ── v14: PIER POSITIONS ────────────────────────────────────────────────────────
# house-side row still dropped (from v13); outer row shifts in 3" with frame
# middle row: y=8'-1.5" = 8.125 ft (unchanged)
# outer row:  y=16'-0" = 16.0 ft (was 16'-3" = 16.25 ft in v13)
PIER_Y     = [8.125, FRAME_D]   # outer row now at FRAME_D = 16.0 ft
...
# ── FAR-EDGE STEP dimensions (v13, ribbon y shifts with rim in v14) ────────────
# Step runs along outer 23'-6" edge (far side, y = OVERALL_D = 16.25 ft in v14)
...
# v14: outer PF edge at OVERALL_D = 16.25 ft (was 16.5 ft in v13)
FAR_STEP_Y_ATTACH = OVERALL_D   # = 16.25 ft
...
# Gravel pad: sits beyond y=OVERALL_D = 16.25 ft
FAR_GRAVEL_Y0 = OVERALL_D
FAR_GRAVEL_Y1 = FAR_GRAVEL_Y0 + FAR_TREAD_DEPTH + 4.0/12 + 0.2

# 5 precast blocks, x positions (spec unchanged — absolute positions stay same)
...
# Drop-board y position (outer rim at FRAME_D = 16.0 ft in v14)
DROP_T = 0.085
far_drop_y = RIM_Y + JOIST_H   # = 16.0 + 0.085 = 16.085 ft
FAR_DROP_BOARD_FACE = far_drop_y + DROP_T + FASCIA_T

# Ribbon attaches to far fascia outer face
fas_y = far_drop_y + DROP_T            # = 16.17 ft
fas_y_end = fas_y + FASCIA_T           # = 16.226 ft
...
# ══════════════════════════════════════════════════════════════════════════════
# ══ VERSION BADGE — upper-left corner (v15) ══════════════════════════════════
# ══════════════════════════════════════════════════════════════════════════════
BADGE_X = -6.4
BADGE_Y = -5.4
BADGE_W = 3.6
...
# ── Full patio background (23'-6" × 16'-6") ───────────────────────────────────
# The patio is fixed at 23.5 × 16.5 ft; deck now ends at 16.25 ft (y = OVERALL_D)
# Show full patio underneath everything
...
# ── v14: Exposed patio strip (y = OVERALL_D to PATIO_D = 16'-3" to 16'-6") ───
# This 3" strip is exposed concrete between deck outer edge and patio edge
EXPOSED_Y0 = OVERALL_D   # = 16.25 ft = 16'-3"
EXPOSED_Y1 = PATIO_D      # = 16.5 ft = 16'-6"
EXPOSED_H  = EXPOSED_Y1 - EXPOSED_Y0  # = 0.25 ft = 3"
...
# Cross-hatch to distinguish exposed patio from deck
...
# Dimension arrow for the 3" strip
...
label(strip_ann_x + 0.55, (EXPOSED_Y0 + EXPOSED_Y1)/2,
      "3\"",
      fs=7.0, bold=True, color="#5a4a38", rotation=90, ha="center")
...
# ── Ledger (background) — v13/v14: carries point loads at girder connections ───
LEDGER_H = 0.18
rect(0.0, 0.0, FRAME_W, LEDGER_H, C_LEDGER, EC_LEDGER, lw=2.5, zorder=5, alpha=FRAME_ALPHA)

# ── v13/v14: Hanger symbols at girder-to-ledger connections (3 locations) ─────
HANGER_LOCATIONS = GIRDER_X
HANGER_SZ = 0.22
...
# ── v13/v14: Ledger fastener callout box ──────────────────────────────────────
...
# v14: 16 joist rows at 1'-0" spacing within the 16'-0" frame depth
all_rows = [(i * JOIST_SPACING, False) for i in range(1, 16)]
...
# ── Piers (background) — v14: 6 piers, outer row at y=16'-0" ──────────────────
PIER_R  = 0.22
HOLE_R  = 0.35
...
label(FRAME_W/2, PIER_Y[0] - 0.45,
      "6 PIERS (v14: house-side row still dropped; outer row at y=16'-0\" matches 16-ft framing)",
...
# ══════════════════════════════════════════════════════════════════════════════
# ══ FAR-EDGE STEP — Plan view (v13, ribbon y updated with rim shift in v14) ══
# ══════════════════════════════════════════════════════════════════════════════

FAR_STEP_TREAD_Y0 = OVERALL_D                         # = 16.25 ft (v14 outer PF face)
...
RIBBON_T   = 2 * (1.5/12)  # two 2×6 = 3" = 0.25 ft
...
FAR_PF_Y0 = FRAME_D + PF_OVR - PF_W   # = OVERALL_D - PF_W = 16.25 - 0.45 = 15.8 ft
FAR_PF_Y1 = FRAME_D + PF_OVR          # = OVERALL_D = 16.25 ft
...
# Overall depth annotations — v14 shows both OVERALL_D and PATIO_D
...
# Patio total 16'-6" — shown with lighter annotation
...
# ── v14: outer rim y-coordinate callout ───────────────────────────────────────
# Show outer rim at y=16'-0" with a leader
ax.annotate("outer rim y=16'-0\"\n(v14: -3\" from v13)",
            xy=(FRAME_W/2, FRAME_D),
            xytext=(FRAME_W/2 + 5.5, FRAME_D - 0.5),
            fontsize=6.5, ha="left", color=EC_RIM, fontweight="bold",
            arrowprops=dict(arrowstyle="->", color=EC_RIM, lw=1.1),
            zorder=20, clip_on=False,
            bbox=dict(boxstyle="round,pad=0.2", facecolor="#fffff0", edgecolor=EC_RIM, lw=1.2))

# ══════════════════════════════════════════════════════════════════════════════
# ══ LEFT-EDGE STAIR — SIDE ELEVATION INSET ════════════════════════════════════
# ══ (graphically rendered; was present in render_v13.py but orphaned in drawio)
# ══════════════════════════════════════════════════════════════════════════════
PX = 29.5
PW = 5.5
...
label(EL_X + EL_W/2, EL_Y0 + 0.25,
      "LEFT-EDGE STAIR — SIDE ELEVATION (v12, unchanged)",
...
# ══════════════════════════════════════════════════════════════════════════════
# ══ FAR-EDGE STEP — SIDE ELEVATION INSET (v13, carried to v14) ════════════════
# ══════════════════════════════════════════════════════════════════════════════
FAR_EL_X  = PX + 0.2
FAR_EL_Y0 = 14.5
FAR_EL_W  = PW - 0.4
...
label(FAR_EL_X + FAR_EL_W/2, FAR_EL_Y0 + 0.25,
      "FAR-EDGE STEP — SIDE ELEVATION (v13, v14 unchanged)",
...
title_lines = [
    ...
    "(v15: drawio stair-elevation cells replaced with native shapes;",
    " PNG unchanged from v14 except version badge)",
]
...
count_lines = [
    ...
    ("PF — left side:", "2 pcs: 0-6' + 9'-6\"-16'-3\" (interrupted)"),   # v14: 16'-6"→16'-3"
    ("PF — right side:", "1 board, 16'-3\" length"),                       # v14: 16'-6"→16'-3"
...
# ── Legend ─────────────────────────────────────────────────────────────────────
panel_box(7.2, 8.0, fc="white")
label(PX + PW/2, 7.45, "LEGEND — PAGE 2: DECKING (v14)", fs=10.0, bold=True)
...
label(PX + 0.78, y_leg + 0.09, "Far-edge tread planks — 2× MoistureShield Vision 5.4\"", fs=6.8, bold=True, ha="left")
...
label(PX + 0.78, y_leg + 0.09, "Simpson HU210-3 (or eq.) — girder-to-ledger hanger", fs=6.8, bold=True, ha="left")
label(PX + 0.78, y_leg + 0.26, "3 places; ½\" HDG bolts back-blocked from basement + IRC R507.9.1.3 lags", fs=5.8, ha="left", color="#444444")

y_leg = 13.65
rect(PX + 0.08, y_leg, 0.60, 0.40, C_GIRDER, EC_GIRDER, lw=2, zorder=3, alpha=FRAME_ALPHA)
label(PX + 0.78, y_leg + 0.09, "Framing (background, 40% opacity) — 6 piers, outer row at y=16'-0\"", fs=6.8, bold=True, ha="left")
label(PX + 0.78, y_leg + 0.26, "House-side pier row dropped; middle (y=8'-1.5\") + outer (y=16'-0\") rows remain", fs=5.8, ha="left", color="#444444")

y_leg = 14.45
rect(PX + 0.08, y_leg, 0.60, 0.40, C_PATIO_EXPOSED, EC_PATIO_EXPOSED, lw=1.5, zorder=3, alpha=0.85)
label(PX + 0.78, y_leg + 0.09, "Exposed patio strip — 3\" (y=16'-3\" to y=16'-6\") — added v14, carried v15", fs=6.8, bold=True, ha="left")
label(PX + 0.78, y_leg + 0.26, "Deck does NOT reach patio far edge; 3\" of bare concrete visible along far side", fs=5.8, ha="left", color="#5a4a38")
...
print(f"FRAME_D (framing depth):      {FRAME_D:.4f} ft = {FRAME_D*12:.2f}\" = {int(FRAME_D)}'-{(FRAME_D*12)%12:.2f}\"")
print(f"OVERALL_D (PF outer edge):    {OVERALL_D:.4f} ft = {OVERALL_D*12:.2f}\" = {int(OVERALL_D)}'-{(OVERALL_D*12)%12:.2f}\"")
print(f"PATIO_D (patio edge):         {PATIO_D:.4f} ft = {PATIO_D*12:.2f}\"")
print(f"Exposed patio strip:          {(PATIO_D - OVERALL_D)*12:.2f}\"")
print(f"Outer pier row y:             {PIER_Y[1]:.4f} ft = {PIER_Y[1]*12:.2f}\"")
print(f"FAR_STEP_Y_ATTACH (ribbon y): {FAR_STEP_Y_ATTACH:.4f} ft")
...
print(f"Pier rows: {PIER_Y}  (6 piers total, house-side row still dropped)")
print(f"PF cantilever past rim: {PF_OVR*12:.2f}\" (should be 3.00)")
print(f"PF cantilever past fascia: {(PF_OVR - FASCIA_T)*12:.2f}\" (should be 2.33)")
