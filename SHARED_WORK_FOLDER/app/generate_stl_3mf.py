...
# ---------------------------------------------------------------------------
# Fringe-area iso-line + arrow extraction (v4.32 experiment, Topo 2026-09-02)
# ---------------------------------------------------------------------------
#
# Thomas's ask (task 711): the GolfLogix source images render thin light-colored
# iso-lines and small dark arrows in the fringe area (outside the green
# boundary). The green pipeline uses the arrows and (implicitly) iso-lines
# inside the green to build the green's elevation surface via Poisson solve.
# The fringe currently ignores everything outside the green boundary and is
# built as a nearly-flat cap that just holds the green-edge Z. This function
# extracts the fringe-area iso-lines and arrows so we can give the fringe some
# Z-value character.
#
# Approach: build a fringe pixel mask (image bounds minus green polygon minus
# trap/water polygons), then run two detectors:
#
#   • Iso-line detection: iso-lines are LIGHT (white / off-white) thin lines on
#     the DARKER green grass background. Threshold HSV value > 180 inside the
#     fringe mask, keep only thin components (area 40 – 4000 px, length >> width).
#     Return the pixel positions as scattered elevation-reference points.
#
#   • Arrow detection: reuse the same "small dark blob" heuristic as inside the
#     green. Return centroid pixel coords.
#
# Both outputs are pixel coords — the caller (build_fringe_mesh) is responsible
# for converting to mm and interpolating a Z field.
#
# This function is a NEW extraction path. The existing green pipeline is
# untouched. If no iso-lines or arrows are found in the fringe (older/simpler
# images) the returned arrays are empty and the fringe stays flat — full
# backward compatibility.


def extract_fringe_terrain_from_image(
    image_path: str,
    green_boundary_px: np.ndarray,
    egm_data: dict,
    debug_out: str | None = None,
    ...
    """
    Extract fringe-area iso-line pixels + arrow centroids from a source image.

    Parameters
    ----------
    image_path         : Absolute path to the source PNG/JPG.
    green_boundary_px  : Nx2 float array of green polygon vertices in image
                         pixel coords.
    egm_data           : Full EGM dict (needed for trap + water polygon exclusion).
    debug_out          : Optional path to save the debug visualisation PNG.
                         Defaults to owner_inbox/debug_fringe_terrain.png.

    Returns
    -------
    dict with keys:
        isoline_pixels  : Nx2 int array of (x, y) pixel positions on iso-lines
        arrow_pixels    : Mx2 int array of (x, y) pixel positions of arrow centroids
        fringe_mask     : uint8 HxW binary mask of the fringe region used
    ...
    img = cv2.imread(image_path)
    if img is None:
        raise FileNotFoundError(f"[fringe_grad] cannot read image: {image_path}")

    h, w = img.shape[:2]

    # ── Build fringe pixel mask ──────────────────────────────────────────────
    # Start with the full image, subtract green polygon and every trap/water
    # polygon. What's left is where we look for iso-lines and arrows.
    fringe_mask = np.full((h, w), 255, dtype=np.uint8)

    # Subtract green polygon
    boundary_i = green_boundary_px.astype(np.int32)
    cv2.fillPoly(fringe_mask, [boundary_i], 0)

    # Subtract every trap and water polygon
    n_traps = 0
    n_water = 0
    for poly in egm_data.get("polygons", []):
        ...
        if len(pts) < 3:
            continue
        pts_px = np.array(
            [[p["x"], p["y"]] for p in pts], dtype=np.int32
        )
        cv2.fillPoly(fringe_mask, [pts_px], 0)
        if ptype == "trap":
            n_traps += 1
        else:
            n_water += 1

    # Also subtract a small border to avoid image-edge artifacts
    border = 4
    fringe_mask[:border, :] = 0
    fringe_mask[-border:, :] = 0
    fringe_mask[:, :border] = 0
    fringe_mask[:, -border:] = 0

    print(f"[fringe_grad] fringe mask built: green + {n_traps} trap(s) + {n_water} water subtracted")
    print(f"[fringe_grad] fringe pixel count: {int((fringe_mask == 255).sum())} / {h * w}")

    # ── Detect iso-lines (light thin lines on darker grass) ──────────────────
    # In HSV, iso-lines are high V (bright). The fringe grass is medium V (~80-140).
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    value = hsv[:, :, 2]
    sat = hsv[:, :, 1]

    # Iso-lines: bright + low-saturation (white/gray), inside fringe mask
    ISO_V_MIN = 170
    ISO_S_MAX = 60
    iso_mask = np.zeros((h, w), dtype=np.uint8)
    iso_mask[(value >= ISO_V_MIN) & (sat <= ISO_S_MAX) & (fringe_mask == 255)] = 255

    # Clean up: remove tiny specks and connect broken lines
    kernel3 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    iso_mask = cv2.morphologyEx(iso_mask, cv2.MORPH_OPEN, kernel3, iterations=1)

    # Filter connected components: keep only thin, elongated ones (iso-lines,
    # not blobs of white text/labels/UI). A component's linearity is measured
    # by the ratio of its bounding-box longest side to its area's square root
    # — thin lines have high ratio, blobs have low ratio.
    n_labels, labels, stats, _ = cv2.connectedComponentsWithStats(iso_mask, connectivity=8)

    ISO_MIN_AREA = 40         # px² — smaller = ignore, larger = candidate
    ISO_MAX_AREA = 8000       # px² — beyond this, likely UI blob (flag icon, etc.)
    ISO_MIN_LINEARITY = 1.5   # bbox_diag / sqrt(area); pure square = 1.41

    keep_mask = np.zeros_like(iso_mask)
    n_kept = 0
    n_rej_area = 0
    n_rej_linearity = 0
    for lbl in range(1, n_labels):
        area = int(stats[lbl, cv2.CC_STAT_AREA])
        if area < ISO_MIN_AREA or area > ISO_MAX_AREA:
            n_rej_area += 1
            continue
        bw = int(stats[lbl, cv2.CC_STAT_WIDTH])
        bh = int(stats[lbl, cv2.CC_STAT_HEIGHT])
        diag = math.sqrt(bw * bw + bh * bh)
        linearity = diag / max(1.0, math.sqrt(area))
        if linearity < ISO_MIN_LINEARITY:
            n_rej_linearity += 1
            continue
    ...
    print(
        f"[fringe_grad] iso-line components: kept={n_kept} "
        f"(rej_area={n_rej_area}, rej_linearity={n_rej_linearity})"
    )

    ys_iso, xs_iso = np.where(keep_mask == 255)
    isoline_pixels = np.column_stack([xs_iso, ys_iso]).astype(np.int32)
    print(f"[fringe_grad] iso-line pixel count: {len(isoline_pixels)}")

    # ── Detect arrows in fringe (small dark isolated blobs) ──────────────────
    ARROW_V_MAX = 60
    ARROW_MIN_AREA = 20
    ARROW_MAX_AREA = 400
    dark_mask = np.zeros((h, w), dtype=np.uint8)
    dark_mask[(value < ARROW_V_MAX) & (fringe_mask == 255)] = 255

    n_lbl_a, lbl_a, stats_a, cent_a = cv2.connectedComponentsWithStats(dark_mask, connectivity=8)
    arrow_pixels_list: list[tuple[int, int]] = []
    n_arrow_rej = 0
    for lbl in range(1, n_lbl_a):
        area = int(stats_a[lbl, cv2.CC_STAT_AREA])
        if area < ARROW_MIN_AREA or area > ARROW_MAX_AREA:
            n_arrow_rej += 1
            continue
        cx, cy = cent_a[lbl]
        arrow_pixels_list.append((int(round(cx)), int(round(cy))))

    arrow_pixels = np.array(arrow_pixels_list, dtype=np.int32) if arrow_pixels_list else np.zeros((0, 2), dtype=np.int32)
    print(f"[fringe_grad] fringe arrow count: {len(arrow_pixels)} (rejected {n_arrow_rej} by area)")

    # ── Debug image ──────────────────────────────────────────────────────────
    if debug_out is None:
        debug_out = os.path.join(OWNER_INBOX, "debug_fringe_terrain.png")
    try:
        os.makedirs(os.path.dirname(debug_out), exist_ok=True)
        vis = img.copy()
        # Dim non-fringe area
        dim = (vis.astype(np.int32) * 40 // 100).clip(0, 255).astype(np.uint8)
        vis[fringe_mask == 0] = dim[fringe_mask == 0]
        # Draw iso-line pixels in bright cyan
        vis[keep_mask == 255] = (255, 255, 0)
        # Draw arrows as red circles
        for (ax_, ay_) in arrow_pixels_list:
            cv2.circle(vis, (ax_, ay_), 4, (0, 0, 255), -1)
        # Green boundary in green
        cv2.polylines(vis, [boundary_i], isClosed=True, color=(0, 255, 0), thickness=2)
        cv2.putText(vis, f"iso pts: {len(isoline_pixels)}  arrows: {len(arrow_pixels)}",
                    (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        cv2.imwrite(debug_out, vis)
        print(f"[fringe_grad] debug image saved: {debug_out}")
    except Exception as exc:
        print(f"[fringe_grad] debug image failed: {exc}")

    return {
        "isoline_pixels": isoline_pixels,
