"""
state.py — Idempotency state for mac_calendar_bridge.

Tracks which event_ids have already been pushed to Calendar.app.
Persisted as JSONL at ~/.local/share/mac_calendar_bridge/pushed.jsonl
...
class PushedState:
    """
    Tracks event_ids that have been successfully pushed to Calendar.app.

    Usage::

        state = PushedState(Path("~/.local/share/mac_calendar_bridge/pushed.jsonl"))
        if not state.is_pushed("2026-09-15-meeting-with-myself"):
            # push it ...
    ...
    def reset_from_ids(self, event_ids: set[str]) -> None:
        ...
        Used after a slow Calendar.app query to rebuild state without
