"""
render_v9.py  —  Gemma's matplotlib render of deck_plan v9  (Page 2: Decking Plan)
Produces deck_plan.png at >=2000 px wide, white background.

v9 = v8 content unchanged + version badge in upper-left corner.
This script renders PAGE 2 (Decking Plan), which shows:
  - Version badge "v9 / Page 2 of 2 — Decking" in the upper-left corner
  ...
  - Updated title block and legend for v9 / Page 2
...
Plank math (verified):
  ...
  Total boards: 48 field (16-ft) + 4 PF boards → ~52; +10% waste → order ~57
"""
...
# Trex decking colors (new for v8/v9)
...
# ══════════════════════════════════════════════════════════════════════════════
# ══ VERSION BADGE — upper-left corner (v9 addition) ═════════════════════════
# ══════════════════════════════════════════════════════════════════════════════
# Badge position: top-left of the plot area, above and left of the deck plan.
# The plot x range is -4.5..34.5 (39 units wide); y inverted, range -4.5..24.0.
# After invert_yaxis, y=-4.5 is the TOP of the figure. Badge goes at top-left.
BADGE_X = -4.4   # left edge
BADGE_Y = -4.4   # top (remember y axis is inverted, so -4.4 is topmost)
BADGE_W = 3.6
BADGE_H = 1.8

# Dark-filled badge box
...
lx = -PF_OVR - FASCIA_T       # left outer edge of PF
...
BL_x0 = LEFT_PF_X0;  BL_x1 = LEFT_PF_X1
BL_y0 = FAR_PF_Y0;   BL_y1 = FAR_PF_Y1
...
order_notes = [
    "DECKING ORDERING NOTES (planning est. — v9)",
