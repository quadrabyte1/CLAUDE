"""
render_v10.py  —  Gemma's matplotlib render of deck_plan v10  (Page 2: Decking Plan)
Produces deck_plan.png at >=2000 px wide, white background.

v10 = v9 + outside-corner mitered joint correctly rendered at lower-left and lower-right.
Changes from v9:
  - LEFT PF and RIGHT PF planks trimmed at the far-side PF inner edge (y=FAR_PF_Y0).
  - Corner overlap squares at lower-left and lower-right filled with explicit
    triangular polygon patches so the miter diagonal is solid and gap-free.
  - Far fascia extended to include the full corner (left/right fascia extended
    to y=fas_y+FASCIA_T so the butt-join is flush, no empty wedge).
  - Version badge updated to "v10".
  - Title block updated to "DECK PLAN v10".
...
Plank math (verified, unchanged from v8/v9):
  Field planks:
    Width: 5.5" = 0.4583 ft
    Gap: 1/8" = 0.01042 ft
    Effective spacing center-to-center: 5.625" = 0.46875 ft
    Length: 15'-7" = 187" (house-side PF outer edge y=5.5" to far-side PF inner edge)
    Field area width: 22'-7" = 271" (framing 23'-0" minus 2 × 2.5" PF inboard overlap)
    Count: 271" / 5.625" = 48.18 → 48 planks
  PF planks (all 5.5" wide = 0.4583 ft):
    House-side: 23'-6" long, y=0..5.5" from wall
...
# ══════════════════════════════════════════════════════════════════════════════
# ══ VERSION BADGE — upper-left corner (v10) ══════════════════════════════════
# ══════════════════════════════════════════════════════════════════════════════
BADGE_X = -4.4
BADGE_Y = -4.4
BADGE_W = 3.6
...
# ── Fascia (background) ───────────────────────────────────────────────────────
# v10 fix: extend left/right fascia strips to include the far fascia height
# so there is no open gap at the outer lower corners.
...
# ── LEFT PF — v10 fix: draw as mitered polygon, NOT a full rectangle ──────────
...
# This gives a 5-vertex polygon that covers the full left PF band including the
...
# ── RIGHT PF — v10 fix: mirrored 5-vertex polygon ────────────────────────────
...
# ── Miter fill triangles — fill the corner squares with no gap ────────────────
# The corner overlap square at lower-left is:
#   x: [LEFT_PF_X0, LEFT_PF_X1]
#   y: [FAR_PF_Y0, FAR_PF_Y1]
# The left PF polygon already covers the upper-left triangle of this square
# (its last edge goes from (LEFT_PF_X1, FAR_PF_Y0) to (LEFT_PF_X0, FAR_PF_Y1)).
# The far PF rectangle covers the full width including this square.
# So both PF boards together fill the entire corner square:
#   - far PF rect covers it entirely (it's drawn at full width)
#   - left PF polygon covers the upper-right triangle (leaving the lower-left
#     triangle to the far PF)
# The miter seam line (explicit diagonal) is drawn on top.
...
title_lines = [
    ...
    "(v10: outside corner miter fixed at lower-left / lower-right)",
]
...
order_notes = [
    "DECKING ORDERING NOTES (planning est. — v10)",
