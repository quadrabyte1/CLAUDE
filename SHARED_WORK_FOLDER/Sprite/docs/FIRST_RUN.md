...
- [ ] Ollama running with `qwen2.5:7b-instruct` resident (`ollama run qwen2.5:7b-instruct`)
- [ ] whisper-cpp installed (`brew install whisper-cpp`) and model at `~/sprite/models/ggml-small.en.bin`
...
```
INFO sprite.watcher Sprite watcher v0.2.0 starting
...
INFO sprite.transcribe transcribe: running whisper-cpp on <uuid>.m4a
...
| whisper-cpp not found | `brew install whisper-cpp` and verify: `which whisper-cpp` |
