#!/usr/bin/env python3
"""
Generate flat 3D-printable STL pieces from the Moffett Field G.C. Hole 9
...
# -- Paths -----------------------------------------------------------------
INPUT = "/Users/fourierflight/GIT/CLAUDE-GitHub/SHARED_WORK_FOLDER/team_inbox/Moffett Field 9 (GolfLogix).png"
OUTPUT_DIR = "/Users/fourierflight/GIT/CLAUDE-GitHub/SHARED_WORK_FOLDER/owner_inbox"

OUTPUT_GREEN = os.path.join(OUTPUT_DIR, "moffett_9_green.stl")
OUTPUT_FRINGE = os.path.join(OUTPUT_DIR, "moffett_9_fringe.stl")
OUTPUT_TRAP_1 = os.path.join(OUTPUT_DIR, "moffett_9_trap_1.stl")
OUTPUT_TRAP_2 = os.path.join(OUTPUT_DIR, "moffett_9_trap_2.stl")
OUTPUT_TRAP_3 = os.path.join(OUTPUT_DIR, "moffett_9_trap_3.stl")
...
for ray_idx in range(n_rays):
    ...
    # Scan the ray for ALL dark bands, then take the OUTERMOST one
    ...
    # Hole 9 has a thinner/lighter boundary line than hole 7, so we use a
    # higher dark threshold (50 vs 35) and accept narrower bands (width >= 2).
    DARK_THRESH = 50
    MIN_DARK_WIDTH = 2
    candidates = []
    i = 0
    while i < len(ray_pixels):
        ...
        if g < DARK_THRESH:
            dark_start = i
            while i < len(ray_pixels) and ray_pixels[i][2] < DARK_THRESH:
                i += 1
            dark_width = i - dark_start

            if dark_width >= MIN_DARK_WIDTH and i < len(ray_pixels):
                # Check what's on the far side (look further out — 30 pixels)
                exit_end = min(i + 30, len(ray_pixels))
                exit_pixels = ray_pixels[i:exit_end]
                if exit_pixels:
                    exit_coords = [(p[0], p[1]) for p in exit_pixels]
                    exit_hues = [float(hsv_clean[ey, ex, 0]) for ex, ey in exit_coords
                    ...
                    exit_vals = [float(hsv_clean[ey, ex, 2]) for ex, ey in exit_coords
                                 if 0 <= ey < ch and 0 <= ex < cw]
                    if exit_sats:
                        avg_hue = np.mean(exit_hues)
                        avg_sat = np.mean(exit_sats)
                        avg_val = np.mean(exit_vals)
                        mid_idx = dark_start + dark_width // 2
                        candidates.append((mid_idx, dark_width, avg_hue, avg_sat, avg_val))
        else:
            i += 1

    # Take the outermost candidate that exits to fringe.
    # Fringe is dark forest green: hue 30-90 (green range), moderate sat,
    # and notably darker (lower value) than the colorful contour interior.
    # Also accept low-saturation exits (gray/dark areas beyond boundary).
    boundary_pt = None
    for mid_idx, dw, avg_hue, avg_sat, avg_val in reversed(candidates):
        is_green_fringe = (30 <= avg_hue <= 90) and (avg_val < 160)
        is_dark_exit = (avg_val < 120)
        is_low_sat = (avg_sat < 80)
        is_fringe = is_green_fringe or is_dark_exit or is_low_sat
        if is_fringe:
    ...
    # Debug: print info for every 90th ray (8 sample directions)
    ray_angle_deg = 360.0 * ray_idx / n_rays
    if ray_idx % 90 == 0:
        if boundary_pt:
            dist_from_seed = np.sqrt((boundary_pt[0] - seed_x)**2 + (boundary_pt[1] - seed_y)**2)
            bp_str = f"({boundary_pt[0]},{boundary_pt[1]}) d={dist_from_seed:.0f}"
        else:
            bp_str = "NONE"
        print(f"  Ray {ray_idx} ({ray_angle_deg:.0f}deg): {len(candidates)} dark bands, boundary={bp_str}")
        for ci, (mi, dw, ah, asat, aval) in enumerate(candidates):
            print(f"    band {ci}: width={dw}, exit hue={ah:.0f} sat={asat:.0f} val={aval:.0f}")
...
# Fringe/rough detection: dark forest green surrounding the putting surface.
# For hole 9, the fringe is darker than the fairway but we can afford a
# slightly higher value cap since the fairway stripe exclusion handles
# the bottom region separately.
overall_green = (
    (hue >= 30) & (hue <= 90) &
    (sat >= 25) &                    # slightly lower to catch darker fringe tones
    (val >= 25) & (val <= 175)       # raised cap; fairway stripes handled separately
)
...
# Fringe extends further on hole 9 — needs to encompass the full green complex
# including the area around the sand traps. Use 1.2x green radius.
max_fringe_dist = green_equiv_radius * 1.2
...
# -- Fairway stripe exclusion --
# The fairway occupies the bottom portion of the image and has alternating
# bright/dark mowing stripes. Detect this pattern: the fairway is brighter
# green (higher value) with lower saturation than the fringe, and shows
# horizontal intensity variance from the stripe pattern.
# Strategy: in the bottom 40% of the image, detect bright green areas that
# are NOT part of the dark fringe and exclude them.
fairway_zone_top = int(ch * 0.60)
fairway_mask = np.zeros((ch, cw), dtype=bool)
# Fairway is bright green: hue in green range, higher value than fringe
fairway_bright = (
    (hue >= 30) & (hue <= 90) &
    (val > 100) &              # brighter than dark fringe
    (sat >= 20) & (sat <= 160)
)
# Detect stripe pattern: compute local intensity variance in horizontal bands
gray_for_stripe = cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY).astype(float)
# Compute vertical gradient magnitude — stripes create horizontal edges
vert_grad = np.abs(np.diff(gray_for_stripe, axis=0))
vert_grad = np.vstack([vert_grad, vert_grad[-1:]])  # pad to same size
# Smooth horizontally to get per-row stripe energy
stripe_energy = cv2.GaussianBlur(vert_grad.astype(np.float32), (51, 5), 0)
# High stripe energy in bottom region = fairway
stripe_thresh = np.percentile(stripe_energy[fairway_zone_top:, :], 70)
stripe_region = (stripe_energy > stripe_thresh)
# Only apply in bottom 40% to avoid false positives on the green
fairway_mask[fairway_zone_top:, :] = (
    fairway_bright[fairway_zone_top:, :] & stripe_region[fairway_zone_top:, :]
)
# Dilate to cover gaps between stripes
kernel_fw = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (15, 15))
fairway_u8 = (fairway_mask.astype(np.uint8)) * 255
fairway_u8 = cv2.morphologyEx(fairway_u8, cv2.MORPH_CLOSE, kernel_fw, iterations=3)
fairway_mask = fairway_u8 > 0
# Remove fairway from overall mask
overall_mask = overall_mask & (~fairway_mask)
print(f"  Fairway stripe pixels excluded: {np.count_nonzero(fairway_mask)}")
