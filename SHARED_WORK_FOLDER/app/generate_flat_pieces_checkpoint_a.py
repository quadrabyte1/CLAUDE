...
# -- Paths -----------------------------------------------------------------
INPUT = "/Users/fourierflight/GIT/CLAUDE-GitHub/SHARED_WORK_FOLDER/team_inbox/Moffet 7 (update).jpg"
...
# Crop to the golf hole area (exclude UI elements at top and bottom)
...
# The fringe is the dark/forest green area. It has:
# - Green hue (hue ~35-85)
# - Lower saturation than the contour lines
# - Moderate value (darker than the bright contour colors)
# We also include the area with ISO lines (gray lines on dark green)

# Broad green detection for the overall area
overall_green = (
    (hue >= 30) & (hue <= 90) &
    (sat >= 20) &
    (val >= 30) & (val <= 200)
...
# Also pick up the gray ISO lines that run through the fringe
gray_lines = (sat < 40) & (val >= 50) & (val <= 160)
...
# Close and fill
kernel_fringe = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (15, 15))
fringe_u8 = cv2.morphologyEx(fringe_u8, cv2.MORPH_CLOSE, kernel_fringe, iterations=5)
