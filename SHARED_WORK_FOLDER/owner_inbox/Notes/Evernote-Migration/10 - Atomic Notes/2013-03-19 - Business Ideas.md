...
- ☐
  ...
  Name idea: BackOfficeTherapyAllow the option to save the user's password, even though we will claim it *never* moves off their local machineSabin pointed out that lots of folks lose passwords and would want the option to have it stored just in caseHand written notesMartha needs a way to save hand written notesuse the Livescribe pencan the audio recording be turned off? pages could be automatically assigned to sessions by recording date against session entries
